...
class DVC(object):
        ...
        with open(filename, "r") as f:
            ...
            if len(config.keys()) != len(self.run_keywords):
                ...
                if len(missing) > 0:
                    ...
                    # the mechanism to pass the point cloud is by passing
                    # the DataCloud object, where roi is a numpy array:
                    # pc = dvcw.DataCloud()
                    # pc.loadPointCloudFromNumpy(roi)
                    if 'point_cloud_filename' in config.keys():
    ...
    def check_input_files(self):
        ...
        npyheader = eval(hd[0])
        hd2 = self.run.parse_npy(1)
        npyheader2 = eval(hd2[0])
        
        if not npyheader == npyheader2:
            raise ValueError('Incompatible input images {0} {1}'
                             .format(npyheader, npyheader2))
    ...
    # config with Python calls directly
    @property
    def reference_filename(self):
        return self.run.ref_fname
    @reference_filename.setter
    def reference_filename(self, filename):
        if os.path.isfile(os.path.abspath(filename)):
            self.run.ref_fname = filename
        else:
            raise ValueError('File {0} does not exist!'
                             .format(os.path.abspath(filename)))
                    
        if os.path.isfile(os.path.abspath(self.run.cor_fname)):
            self.check_input_files()
    @property
    def correlate_filename(self):
        return self.run.ref_fname
    @correlate_filename.setter
    def correlate_filename(self, filename):
        if os.path.isfile(os.path.abspath(filename)):
            self.run.cor_fname = filename
        else:
            raise ValueError('File {0} does not exist!'
                             .format(os.path.abspath(filename)))
                    
        if os.path.isfile(os.path.abspath(self.run.ref_fname)):
            self.check_input_files()
                    
                    
    @property
    def output_filename(self):
        return {'output_filename' : self.run.out_fname , 
                'result_filename' : self.run.res_fname ,
                'statistics_filename' : self.run.sta_fname }
    @output_filename.setter
    def output_filename(self, filename):    
        self.run.out_fname = filename 
        self.run.res_fname = filename + ".disp"
        self.run.sta_fname = filename + ".stat"
    
        
        
    @property
    def subvol_geometry(self):
        pass
    @subvol_geometry.setter
    def subvol_geometry(self, geometry):
        if geometry.lower() == 'cube':
            sg = dvcw.Subvol_Type.cube
        elif geometry.lower() == 'sphere':
            sg = dvcw.Subvol_Type.sphere
        else:
            #tb = sys.exc_info()[2]
            raise ValueError(
